...
RSpec.describe "Tables", type: :request do
  describe "GET /api/v1/restaurants/:restaurant_id/tables/occupied" do
    ...
    it "returns occupied tables for the specified time" do
      get api_v1_restaurant_occupied_tables_path(restaurant), params: { time: requested_time }
    ...
    it "returns no occupied tables outside reservation times" do
      get api_v1_restaurant_occupied_tables_path(restaurant), params: { time: 3.hours.from_now }
      expect(response).to have_http_status(:ok)
      occupied_tables = JSON.parse(response.body)
